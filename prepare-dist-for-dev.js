...
function prepareDistForDev() {
  ...
  if (!fs.existsSync(path.join(distFolder, 'main.js'))) {
    fs.writeFileSync(path.join(distFolder, 'main.js'), `// ${message}`);
  }

  if (!fs.existsSync(path.join(distFolder, 'style.css'))) {
    fs.writeFileSync(path.join(distFolder, 'style.css'), `/* ${message} */`);
  }

  if (!fs.existsSync(path.join(distFolder, 'kmuld-customizable-currencies-5e.lock'))) {
    fs.writeFileSync(path.join(distFolder, 'kmuld-customizable-currencies-5e.lock'), '🔒');
  }
}
